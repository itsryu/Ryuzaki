...
export default class PayCommand extends CommandStructure {
    ...
    public async commandExecute({ message, args, language }: { message: Message, args: string[], language: Languages }) {
        ...
        if (!member) {
            ...
            return void message.reply({ content: 'Você deve inserir a quantia que deseja enviar.' });
        } else {
            ...
            if (isNaN(money)) {
                ...
                return void message.reply({ content: 'Esta quantia é invalida, pois é menor ou igual a 0. Insira um \`valor válido\`.' });
            } else if (member.id === message.author.id) {
                return void message.reply({ content: 'Você não pode enviar o seu dinheiro para si mesmo.' });
            } else if (money > userData.economy.bank) {
                return void message.reply({ content: 'Você não possui a quantia especificada.' });
            } else {
