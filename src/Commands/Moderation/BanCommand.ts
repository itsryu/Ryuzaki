                        ...
                        if (interaction.customId === 'yes') {
                            ...
                            member.ban({ reason: reason, deleteMessageSeconds: 604800 })
                                .then(async () => {
                                    ...
                                    await msg.edit({ embeds: [banEmbed], components: [] });
                                })
                                .catch(async () => {
                                    await msg.edit({ content: this.client.t('moderation:ban:errors.failed'), embeds: [], components: [] });
                                });
                        }

                        if (interaction.customId === 'no') {
                            await msg.edit({ content: this.client.t('moderation:ban:errors.rejected', { member: member.user.tag }), embeds: [], components: [] });
                        }
                    });
                }
            }
